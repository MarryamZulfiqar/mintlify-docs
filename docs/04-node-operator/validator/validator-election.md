---
id: node-validator-election
title: "Validator Election Process on the Core Network"
sidebar_label: "Validator Election"
sidebar_position: 3
description: "Validators are a crucial part of the Core network."
keywords: ["validator", "election", "process", "on"]
tags: ["validator", "staking"]
hide_table_of_contents: false
pagination_next: "register-validator-node"
pagination_prev: "setup-validator-node"
---
...
* **Decentralization**: The election process allows miners and stakers to participate in [Validator](/docs/07-reference/glossary#v) selection, promoting decentralization and preventing the concentration of control.
...
Where, $$ m + k + l = 1 $$
   ...
   * $$l$$: Ratio assigned to BTC staking.
   ...
   Leaving aside the mathematical details, this is essentially a weighted, bicameral voting procedure. Bitcoin miners can vote for validators through their PoW (by writing validator information into the coinbase transaction on blocks they’ve already mined), CORE token holders can vote for a validator with their PoS (by delegating their tokens to it), and self-custodial Bitcoin stakers can vote for a validator through the delegation of their tokens. This includes delegated Proof-of-Work (PoW), delegated Proof-of-Stake (PoS), and Self-Custodial [Bitcoin Staking](/docs/07-reference/glossary#b), which are weighted to determine the hybrid score.
   
   This is the “core” of the Core blockchain, the mechanism by which the network leverages the security and decentralization of the Bitcoin network and the scalability and composability of PoS chains like Ethereum. Allowing Bitcoin miners and Bitcoin stakers to vote on validators enables Core to leverage Bitcoin’s legendary robustness. Because Core is [EVM compatible](/docs/07-reference/glossary#e), it’s possible to build smart contracts, dApps, and other applications on Core that couldn’t be done without changes to the underlying Bitcoin protocol.
...
7. **Reward Distribution**:
   - Rewards are distributed based on contributions to network security, using the following formula:
      ...
      Where:
      * $$rH$$: Rewards attributed to delegated hash power ([DPoW](/docs/07-reference/glossary#d)).
      * $$rS$$: Rewards attributed to CORE staking ([DPoS](/docs/07-reference/glossary#d)).
...
With the introduction of [Dual Staking](/docs/07-reference/glossary#d), BTC staking rewards are now tiered based on the amount of CORE staked relative to BTC. BTC rewards are no longer evenly distributed across all participants. Instead, they are allocated dynamically based on dual staking thresholds, with higher tiers generally receiving a greater proportion of the rewards. This tiered structure introduces variability in BTC staking returns, aligning incentives across the Core ecosystem while maintaining proportionality in reward distribution.
...
- [Validator Overview](/docs/validator-overview)
- [Setting Up a Validator Node](/docs/setup-validator-node)
- [Slashing & Jailing](/docs/[slashing](/docs/07-reference/glossary#s)-overview)

...
title: "Glossary"
slug: glossary
description: "Authoritative definitions for all Z Protocol terms. Organized by category. When terminology is in doubt, this page is the source of truth."
...
ai_summary: |
  Canonical Z Protocol terminology. Protocol: Z (the protocol and token when clear from context), Z Protocol (formal name), Zcosystem (application layer), ZApp (individual application), Zed (anon founder). Privacy: MASP = Multi-Asset Shielded Pool (UTXO note system, Poseidon commitments in Merkle tree, Groth16 verified); ShieldedPool = same as MASP; note = private UTXO inside MASP; commitment = Poseidon hash of note contents; nullifier = Poseidon(nsk, leafIndex) published on spend; stealth address = one-time ERC-5564 ECDH address for DeFi; viewing key = read-only key derived from masterKey; spending key = authorizes spending from MASP; shielded = inside MASP specifically (NOT blanket term for all Z privacy); unlinkable = DeFi via stealth address (visible but not traceable); private = general acceptable term for either. Consensus: ZSP, Satoshi Plus, validator, DPoW (Leg 1), self-custodial staking (Leg 2), DPoS (Leg 3), timelock (CLTV output on Zcash — NOT a transaction, ZEC stays in wallet), hybrid score, round (24h), Dual Staking, jailing. Assets: Z token (1B fixed supply), ZEC (Zcash native, gas on Z), USDZ (1:1 USDC/USDT stablecoin), yUSDZ (ERC-4626 yield vault), stZEC (liquid staked ZEC), stZ (liquid staked Z), wZEC (ERC-20 ZEC on Z). Technical: Groth16 (ZK proof system on BN254, NOT Halo 2), BN254 (elliptic curve), Halo 2 (Zcash L1's proof system — DIFFERENT from Z), Poseidon (ZK-optimized hash for BN254), Merkle tree (depth 16, 65536 leaves), ERC-4337, ERC-5564, Paymaster, RelayAdapter, Broadcaster, Relayer, SPV, Equihash (precompile 0x0B), Blake2b (precompile 0x0C). Agent: MCP, SKILL.md, KYA. Bridge: SPV Bridge, LayerZero, FROST, Direct-to-MASP deposit.
---
...
| **Viewing key** | A cryptographic key derived from the master key that allows a third party to see a user's MASP balances and transaction history without the ability to spend. Used for compliance, auditing, and selective disclosure. Derived: `Poseidon(masterKey, 1)`. |
...
| **Timelock** | A CLTV (CheckLockTimeVerify) output that locks ZEC for a chosen duration. Used for self-custodial staking. Not a transaction in the sense of sending ZEC anywhere — the ZEC stays in the holder's wallet. Voting and reward metadata are embedded in the timelock metadata. |
...
| Term | Definition |
|------|-----------|
| **Z token** | The governance and value-capture token for Z Protocol. Fixed supply of 1 billion. Used for gas payments, staking (Leg 3 of ZSP), governance, and value accrual from Zcosystem revenue. |
...
| **yUSDZ** | Yield-bearing version of USDZ. An ERC-4626 vault where the exchange rate appreciates as yield accrues. Non-rebasing (balance stays constant, value per token grows). |
| **stZEC** | Liquid staking token representing staked ZEC on Z. Appreciates as consensus rewards accrue. |
...
| **ERC-4337** | Account abstraction standard. Z uses it for agent wallets with owner-controlled policies (spending caps, contract allowlists, time-bounded session keys, kill switches) and for Paymaster contracts that accept ZEC for gas. |
...
| **Paymaster** | An ERC-4337 contract that accepts ZEC for gas, swaps it internally for the Z token, and pays the EVM gas fee. Users never interact with the conversion directly. |
...
| **Broadcaster** | A permissionless participant who submits MASP transactions on behalf of users, abstracting the sender's identity from the transaction. Compensated via a fee deducted from the user's shielded note. |
| **Relayer** | A permissionless participant who submits Zcash block headers and staking transactions to Z's on-chain verification contracts (Zcash Light Client, ZcashStake). Different from a broadcaster. |
...
| **Equihash precompile** | EVM precompile at address `0x0B`. Validates Zcash proof-of-work solutions on-chain. Memory-hard computation with a flat-rate gas cost. Implemented in deterministic Go. |
| **Blake2b precompile** | EVM precompile at address `0x0C`. Blake2b hashing with Zcash personalization strings. Used for Merkle root verification and ZIP-244 transaction ID computation. Dynamic linear fee based on payload size. |
...
| Term | Definition |
|------|-----------|
| **MCP** | Model Context Protocol. The open standard for connecting AI agents to external tools. Z ships one MCP server per ZApp and a protocol-level MCP for cross-app operations. Compatible with Claude Code, Cursor, Windsurf, Codex, and other MCP clients. |
| **SKILL.md** | A structured documentation file designed for LLM and AI agent consumption. Contains walkthroughs, ABIs, contract addresses, and executable code examples. Self-contained — no external references required to complete the described operation. |
| **KYA** | Know Your Agent. A framework for verifying agent identity and authorization level before granting access to protocol functions. |
...
| **FROST** | Flexible Round-Optimized Schnorr Threshold signatures. Planned mechanism for ZEC withdrawals — Z's validator set participates as threshold signers in a t-of-n scheme for outbound ZEC. |
